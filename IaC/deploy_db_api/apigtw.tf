...
### /jupyter, /jupyter/{uid}, /admin resources
...
resource "aws_api_gateway_resource" "jupyter_admin_resource" {
  rest_api_id = aws_api_gateway_rest_api.callisto_db_api.id
  parent_id   = aws_api_gateway_resource.jupyter_resource.id
  path_part   = "admin"
}

### /jupyter, /jupyter/{uid}, /admin methods
...
resource "aws_api_gateway_method" "any_jupyter_admin_method" {
  rest_api_id   = aws_api_gateway_rest_api.callisto_db_api.id
  resource_id   = aws_api_gateway_resource.jupyter_admin_resource.id
  http_method   = "ANY"
  authorization = "COGNITO_USER_POOLS"
  authorizer_id = aws_api_gateway_authorizer.callisto_cognito.id

  request_parameters = {
    "method.request.path.admin" = true
  }
}

resource "aws_api_gateway_method_response" "any_jupyter_admin_method_response" {
  rest_api_id = aws_api_gateway_rest_api.callisto_db_api.id
  resource_id = aws_api_gateway_resource.jupyter_admin_resource.id
  http_method = aws_api_gateway_method.any_jupyter_admin_method.http_method
  status_code = "200"
...
resource "aws_api_gateway_method" "options_jupyter_admin_method" {
  rest_api_id   = aws_api_gateway_rest_api.callisto_db_api.id
  resource_id   = aws_api_gateway_resource.jupyter_admin_resource.id
  http_method   = "OPTIONS"
  authorization = "NONE"
...
resource "aws_api_gateway_method_response" "jupyter_admin_options_lambda_method_response" {
  rest_api_id = aws_api_gateway_rest_api.callisto_db_api.id
  resource_id = aws_api_gateway_resource.jupyter_admin_resource.id
  http_method = aws_api_gateway_method.options_jupyter_admin_method.http_method
  status_code = "200"
  response_parameters = {
    "method.response.header.Access-Control-Allow-Headers"     = true
    "method.response.header.Access-Control-Allow-Methods"     = true
...
resource "aws_api_gateway_integration" "jupyter_admin_any_lambda_integration" {
  rest_api_id             = aws_api_gateway_rest_api.callisto_db_api.id
  resource_id             = aws_api_gateway_resource.jupyter_admin_resource.id
  http_method             = aws_api_gateway_method.any_jupyter_admin_method.http_method
  type                    = "AWS_PROXY"
...
resource "aws_api_gateway_integration" "jupyter_admin_options_lambda_integration" {
  rest_api_id = aws_api_gateway_rest_api.callisto_db_api.id
  resource_id = aws_api_gateway_resource.jupyter_admin_resource.id
  http_method = aws_api_gateway_method.options_jupyter_admin_method.http_method
  type        = "MOCK"
...
resource "aws_api_gateway_integration_response" "jupyter_admin_options_lambda_integration_response" {
  rest_api_id = aws_api_gateway_rest_api.callisto_db_api.id
  resource_id = aws_api_gateway_resource.jupyter_admin_resource.id
  http_method = aws_api_gateway_method.options_jupyter_admin_method.http_method
  status_code = aws_api_gateway_method_response.jupyter_admin_options_lambda_method_response.status_code
...
resource "aws_api_gateway_deployment" "callisto_db_api_deployment" {
  ...
  depends_on = [ aws_api_gateway_integration.jupyter_any_lambda_integration,
                 ...
                 aws_api_gateway_integration.jupyter_admin_any_lambda_integration,
